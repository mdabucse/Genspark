BEGIN;
-- Final clean PostgreSQL script generated from the uploaded SQL Server/Northwind script.
-- Names normalized to lowercase PostgreSQL identifiers; indexes renamed uniquely.
-- SQL Server image/binary sample picture values were already set to NULL for portability.
...
-- Converted to PostgreSQL from the uploaded SQL Server script.
-- Main conversions: IDENTITY -> GENERATED BY DEFAULT AS IDENTITY, nvarchar/nchar/ntext/image/money/datetime -> PostgreSQL equivalents, GO -> semicolon, SQL Server procedures -> PostgreSQL functions.
-- Note: SQL Server image/binary sample picture values were set to NULL for portability.
-- Source: uploaded file pasted_text_1_txt.
...
CREATE TABLE orders (
	...
	freight NUMERIC(19,4) NULL CONSTRAINT df_orders_freight DEFAULT (0),
...
;
 CREATE INDEX idx_orders_customersorders ON orders(customerid)
;
 CREATE INDEX idx_orders_employeeid ON orders(employeeid)
;
 CREATE INDEX idx_orders_employeesorders ON orders(employeeid)
;
...
CREATE TABLE products (
	...
	unitprice NUMERIC(19,4) NULL CONSTRAINT df_products_unitprice DEFAULT (0),
	unitsinstock SMALLINT NULL CONSTRAINT df_products_unitsinstock DEFAULT (0),
	unitsonorder SMALLINT NULL CONSTRAINT df_products_unitsonorder DEFAULT (0),
	reorderlevel SMALLINT NULL CONSTRAINT df_products_reorderlevel DEFAULT (0),
	discontinued SMALLINT NOT NULL CONSTRAINT df_products_discontinued DEFAULT (0),
...
;
 CREATE INDEX idx_products_categoriesproducts ON products(categoryid)
;
...
;
 CREATE INDEX idx_products_suppliersproducts ON products(supplierid)
;
CREATE TABLE order_details (
	orderid INTEGER NOT NULL ,
	productid INTEGER NOT NULL ,
	unitprice NUMERIC(19,4) NOT NULL CONSTRAINT df_order_details_unitprice DEFAULT (0),
...
;
 CREATE INDEX idx_order_details_ordersorder_details ON order_details(orderid)
;
 CREATE INDEX idx_order_details_productid ON order_details(productid)
;
 CREATE INDEX idx_order_details_productsorder_details ON order_details(productid)
;
...
SELECT Orders.ShipName, Orders.ShipAddress, Orders.ShipCity, Orders.ShipRegion, Orders.ShipPostalCode, 
	...
	(CAST((order_details.unitprice * order_details.quantity * (1 - order_details.discount)) AS NUMERIC(19,4))) AS ExtendedPrice, Orders.Freight
...
SELECT order_details.OrderID, order_details.ProductID, Products.ProductName, 
	...
	(CAST((order_details.unitprice * order_details.quantity * (1 - order_details.discount)) AS NUMERIC(19,4))) AS ExtendedPrice
...
SELECT order_details.OrderID, Sum(CAST((order_details.unitprice * order_details.quantity * (1 - order_details.discount)) AS NUMERIC(19,4))) AS Subtotal
FROM order_details
GROUP BY order_details.OrderID
;
create view product_sales_for_1997 AS
SELECT Categories.CategoryName, Products.ProductName, 
Sum(CAST((order_details.unitprice * order_details.quantity * (1 - order_details.discount)) AS NUMERIC(19,4))) AS ProductSales
FROM (Categories INNER JOIN Products ON Categories.CategoryID = Products.CategoryID) 
	INNER JOIN (Orders 
		INNER JOIN order_details ON Orders.OrderID = order_details.OrderID) 
	ON Products.ProductID = order_details.ProductID
...
WHERE (order_subtotals.Subtotal >2500) AND (Orders.ShippedDate BETWEEN DATE '1997-01-01' And DATE '1997-12-31')
;
...
-- Foreign keys for core Northwind tables are added after data load.
-- This avoids load-order problems such as employees.reportsto referencing a later employee row.
...
;
-- PostgreSQL functions converted from SQL Server stored procedures.
CREATE OR REPLACE FUNCTION ten_most_expensive_products()
RETURNS TABLE(tenmostexpensiveproducts VARCHAR(40), unitprice NUMERIC(19,4))
LANGUAGE sql AS $$
...
CREATE OR REPLACE FUNCTION custordersorders(p_customerid CHAR(5))
RETURNS TABLE(orderid INTEGER, orderdate TIMESTAMP, requireddate TIMESTAMP, shippeddate TIMESTAMP)
LANGUAGE sql AS $$
...
CREATE OR REPLACE FUNCTION custorderhist(p_customerid CHAR(5))
RETURNS TABLE(productname VARCHAR(40), total BIGINT)
LANGUAGE sql AS $$
SELECT p.productname, SUM(od.quantity)::BIGINT AS total
FROM products p
...
RETURNS TABLE(productname VARCHAR(40), unitprice NUMERIC, quantity SMALLINT, discount INTEGER, extendedprice NUMERIC)
LANGUAGE sql AS $$
SELECT p.productname,
       ROUND(od.unitprice, 2) AS unitprice,
       od.quantity,
       CAST(od.discount * 100 AS INTEGER) AS discount,
       ROUND(CAST(od.quantity * (1 - od.discount) * od.unitprice AS NUMERIC), 2) AS extendedprice
FROM products p
JOIN order_details od ON od.productid = p.productid
WHERE od.orderid = p_orderid;
$$;

CREATE OR REPLACE FUNCTION salesbycategory(p_categoryname VARCHAR(15), p_ordyear VARCHAR(4) DEFAULT '1998')
RETURNS TABLE(productname VARCHAR(40), totalpurchase NUMERIC)
LANGUAGE plpgsql AS $$
BEGIN
    ...
    RETURN QUERY
    SELECT p.productname,
           ROUND(SUM(CAST(od.quantity * (1 - od.discount) * od.unitprice AS NUMERIC(14,2))), 0) AS totalpurchase
    FROM order_details od
    JOIN orders o ON od.orderid = o.orderid
    JOIN products p ON od.productid = p.productid
    JOIN categories c ON p.categoryid = c.categoryid
    WHERE c.categoryname = p_categoryname
      AND EXTRACT(YEAR FROM o.orderdate)::TEXT = p_ordyear
    GROUP BY p.productname
...
SELECT setval(pg_get_serial_sequence('categories','categoryid'), COALESCE((SELECT MAX(categoryid) FROM categories), 1), true);
SELECT setval(pg_get_serial_sequence('employees','employeeid'), COALESCE((SELECT MAX(employeeid) FROM employees), 1), true);
SELECT setval(pg_get_serial_sequence('shippers','shipperid'), COALESCE((SELECT MAX(shipperid) FROM shippers), 1), true);
SELECT setval(pg_get_serial_sequence('suppliers','supplierid'), COALESCE((SELECT MAX(supplierid) FROM suppliers), 1), true);
SELECT setval(pg_get_serial_sequence('orders','orderid'), COALESCE((SELECT MAX(orderid) FROM orders), 1), true);
